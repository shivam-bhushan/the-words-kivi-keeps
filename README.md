...
`python eval/run_eval.py` — deterministic, no network. Each of the 15 cases
...
Current results (`eval/results/`): 6 useful interventions, 8 correct
abstentions, 1 incorrect intervention, 0 missed corrections — intervention
precision 0.857, recall 1.0, avg latency 0.4 ms, 0 LLM calls, DB ≈ 36 KB
after seeding. The incorrect intervention is left in deliberately: "Kiwi is my
favorite fruit." at sentence start gets capitalized by the formatter, which
destroys the casing signal that protects mid-sentence fruit mentions. The
eval is honest about it rather than curated around it.

## Limitations

- **Sentence-start homographs**: a correction-backed memory will wrongly fire
  on "Kiwi is my favorite fruit." (see above). Fixing this needs semantic
  context, which is a different memory system by design.
